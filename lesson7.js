...
function isBomb(init) {
    var check = false;
    var init_classes = init.getAttribute('class').split(' ');

    if (init_classes.includes('bomb-unit')) {
...
function createBomb() {
    ...
    while (!bombCreated) { //пока прпятствие не создали
        // рандом
        ...
        if (!bomb_cell_classes.includes('food-unit') && !bomb_cell_classes.includes('snake-unit')) {
